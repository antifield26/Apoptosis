...
Totals: **1 583 passed, 0 failed, 35 ignored** across **126 suites**, re-derived from
...
bedrock refusal, early FINISH — Step B), and **eight from P16-06**: one
shape-table spot check, four mover pins (slab step-up with its no-step
control, stair flight, fence containment, headroom trap) and two player
end-to-end (slab stairs walked, fence holds) plus one point-blank bow
positive control (the wall test it guards was rewritten hermetic after the
plant-entombment saga — see the CHANGELOG). The 1 520 -> 1 519 step is the capture sweep
...
other capture-dependent test already follows, and **thirteen from P17-01**:
Step A contributes six `doors` end-to-end (two-half oriented placement,
both-halves toggle with iron refusal, lever-driven open/close, half-break
cleanup, trapdoor face rules, gate swing-back) plus two `mc-redstone` lib
pins (door-family suffixes carrying `open`+`powered`, power-tracking with
manual-state hold); Step B contributes four `mechanisms` end-to-end
(observer pulse with its deaf control, dispenser rising-edge/re-arm,
...
suite: rejoin `JoinGame`s differ and match the server-side player; fails
with the game-side send removed). The round also rewrote the hurt-flash
test onto packet 42 with yaw ≈ 180° (fails with the event-2 arm
restored), moved six socket suites onto `login_join_tick` (the
game-sent `JoinGame` would otherwise deadlock manually-ticked
harnesses), and fixed the view-distance test's join-burst drain — none
...
moves no count. The owner round-2 follow-ups add **three**: orb
magnetism into pickup plus the rest snap (`xp_orbs` 6 -> 8, homing
fails with the steering call removed) and the `@s` self selector
(`admin_commands`, fails addressing a stranger). The Q single-drop
(`inventory_duplication`, fails 10-vs-9 with the status-4 arm removed)
brings the total to **1 581**. The hotbar-sync translation
(`inventory_duplication`, asserts window slot 38 for hotbar 2 — fails
as slot 2 with the index echoed raw) brings it to **1 582**. Dropping
...
client-counter regression case brings it to **1 583**.
...
while updating this total. The 17 lib counts sum to **1 104** (`mc-command` 93,
`mc-core` 28, `mc-test-support` 4, `mc-protocol` 123, `mc-nbt` 17,
`mc-container` 134, `mc-data` 135, `mc-registry` 26, `mc-entity` 158,
...
method. The 5 doc-tests and the named suites complete the 1 577; the
...
per crate, because five of them once turned out to be **another
...
remaining per-crate lib binaries are itemised above and complete the 1 577 total.
