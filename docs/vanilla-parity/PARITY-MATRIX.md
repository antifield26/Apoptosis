...
| Break/place validation | reach, occupied-target and hitbox checks, item consumption | **partial** | `survival_e2e` — 40-block dig refused, placement refused inside a player or an occupied block, survival consumes exactly one item on success. **M-2**: a break now also queues `block_changed_ack` (clientbound play 4) carrying the high-water mark of the client's prediction sequence, which is what lets a real client *apply* the `block_update` it is sent — without it the client stores the change instead of applying it, so the mined block stays visible and every later change at that position is swallowed. **Gap**: a survival dig still breaks instantly on `START_DESTROY_BLOCK` rather than accumulating vanilla's destroy progress per tick (`STOP_DESTROY_BLOCK` takes the same arm), so hardness, tool speed and per-tick progress are not modelled; and no block-specific placement rules, no tool/speed timing, no block-drop items |
...
| Player health/hunger/XP | damage, regen/starve thresholds, three-branch XP formula | **partial** | `mc-entity::player` 28 tests. P11-06 added the player's **10-tick hurt window** (`Session::hurt_invuln_ticks`, `INVULNERABLE_TICKS`), so a mob cannot land two hits in one window; `player_attack.rs` pins the mob side of it. **Gap**: no armour/enchantment/resistance/absorption, no damage typing, no knockback, no effect/attribute system, no XP orbs |
...
| Death/respawn (KD-27) | death screen, respawn, drop inventory | **partial** | Respawn restores vitals. P11-07: the dead player's inventory is drained with `PlayerInventory::drain_all` and spawned as ground item entities at the death position by `Game::after_damage`, so the later `respawn` finds nothing and double-drops nothing; `entity_lifecycle` and the P11-10 acceptance cover the restart half. **M-1**: the packet now carries a whole `CommonPlayerSpawnInfo` plus the trailing data-retention byte, in the order the client's own reader consumes (bytecode-verified; the old body stopped at `is_flat`, so a real client refused it and a dead player could not respawn), and `Session::last_death_location` fills `lastDeathLocation` for real. **Gap**: no XP orbs (experience is reset at respawn with nothing on the ground, because an orb is not an entity kind here), no `keepInventory` game-rule lookup, no spawn-point search, no death screen beyond the `You died!` message, and no respawn-anchor cooldown (the field is sent as `0`) |
...
| Damage/invulnerability | typed sources, i-frames, armour | **partial** | Phase 06 wired `Game::damage_entity` to refuse a hit inside the 10-tick window and scale by `damage_taken_multiplier` (Resistance); P11-02/P11-06 gave the path its production callers — mob melee through `resolve_mob_melee` and player attacks through `PlayIntent::Interact` (kind 1) at `FIST_ATTACK_DAMAGE` — and players now carry the same shared window as `Session::hurt_invuln_ticks`, decremented each tick and checked before a mob hit lands. `player_attack.rs` pins damage, the refusal inside the window and the expiry after it. **Gap**: no damage typing, no armour/enchantment/absorption, no knockback, no fire/drowning/void damage, **a held item's damage is not used** (the fist figure applies whatever is held), **a swing outside the interaction range is not refused**, and the untyped window is not refreshed on a *blocked* hit the way Vanilla's `hurtTime` is |
...
| Data packs: loot/advancements | the remaining data directories | **loot wired and loadable, but the shipped tables mostly refuse — see the evidence** | Loot *loading* exists (`mc-data::loot`) with the real pack's census asserted, and since P11-04 it **is** wired to gameplay: a survival block break rolls `minecraft:blocks/<stem>` and a mob death rolls `minecraft:entities/<kind>` (advancements remain load-only). **The measured problem**: `roll` refuses a whole table when any construct is unmodelled, and the 26.1.2 pack contains such constructs in most tables — `match_tool` **156**, `block_state_property` **149**, `entity_properties` **27**, `killed_by_player` **22**, `table_bonus` **17**, `random_chance_with_enchanted_bonus` **11** (counted from the extracted pack by `target/loot_condition_census.py`; a table with several causes is counted under each). Because the break path supplies **no enchantment levels**, the enchantment-gated tables refuse outright, and the rest refuse on their own unmodelled conditions — so in a real world mining stone and killing a cow yield **nothing**, where vanilla yields cobblestone and leather. P11-04's fixtures prove the wiring only, which is exactly why they could not see this. Two candidate fixes, both player-visible and therefore the owner's call: supply a **known, unenchanted** tool (`Some(empty map)`, which the module's own docs define as "tool known, level 0") instead of `None`, and decide whether an unmodelled condition should refuse the **pool** rather than the table |
