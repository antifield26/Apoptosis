...
| Real-client acceptance (P10-11) | a real client in a real world | **exercised \-- rendering partly verified** | A real 26.1.2 client joined and stayed: colours correct, chat delivered, **and** it kicked the server into fixing two real decode bugs (the bare-string component and the chat_type wire offset). **Still open from the same session**: partial dead-black surface patches (light engine recompute verified in-process; suspect is the mask/array wire encoding — evidence in light_shaft_repro and the vanilla captures) |
