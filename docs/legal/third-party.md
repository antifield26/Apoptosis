...
|---|---|---|
| Pumpkin-master | **GPL-3.0** — `LICENSE:1-2`, `Cargo.toml:113`; plugin-api/utils `MIT OR Apache-2.0` (`crates/pumpkin-plugin-*/Cargo.toml:5`); Bedrock Geyser data MIT (`assets/bedrock/LICENSE-GEYSER:1-3`); data-split notice `assets/NOTICE.md:1-39` | **HIGHEST RISK.** Any copy of GPL-3.0 code (or derivative) into our tree would force GPL-3.0 on the combined work (AGENTS.md §6: no GPL copy without explicit license decision + legal review — none made). Structural learning + behavior observation only. Even "MIT" plugin-api crates live in a GPL workspace — treat as tainted until proven otherwise; do not vendor |
...
| valence-main | **MIT** — `LICENSE.txt:1` (© 2022 Ryan Johnson), `Cargo.toml:109` | Low copyleft risk, but AGENTS.md §6 still requires provenance records for nontrivial derivations + dependency license review. Stale version (1.20.1) limits what we can derive anyway |
...
1. **Clean-room**: no source from any reference clone enters this repo — no copy, no translate, no vendor. Behavior/facts/tests may inform us; code must be independently written. (AGENTS.md §6)
